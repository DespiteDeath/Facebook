...
auto Facebook::Client::check_connection()->bool {
	...
	CURLcode res;
    if(curl) {
    	curl_easy_setopt(curl, CURLOPT_URL, "https://login.facebook.com/login.php");
    	...
    	curl_easy_setopt(curl, CURLOPT_REFERER, "http://facebook.com/login.php");
    	curl_easy_setopt(curl, CURLOPT_FOLLOWLOCATION, 1);
    	curl_easy_setopt(curl, CURLOPT_HEADER, 1);
    	curl_easy_setopt(curl, CURLOPT_SSL_VERIFYPEER, 0);
    	curl_easy_setopt(curl, CURLOPT_SSL_VERIFYHOST, 0);
    	curl_easy_setopt(curl, CURLOPT_POSTFIELDS, "email=Kogut_nikita@mail.ru&pass=try1one2more3");
